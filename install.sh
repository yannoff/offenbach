...
#
# Usage: _download <version> <project> [<exec>] [<asset>]
#
...
#
_download(){
    local version=${1} project=${2} exe=${3} asset=${4} uri
    [ -z "${asset}" ] && asset=${project}
    [ -z "${exe}" ] && exe=${asset}
    _debug "Downloading latest %s release..." "${asset}"
...
then
    ...
    _download latest yamltools ${bindir}/yamltools
fi

# Download the offenbach script
_download ${exe_version} offenbach ${executable}
